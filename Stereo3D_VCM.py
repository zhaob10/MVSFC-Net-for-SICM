...
trainer = edict(
    gpu = 0, # 指定GPU
    max_epochs = 100, # for validation epoch 50 is enough
    disp_iter = 100,
    save_iter = 1,
    test_iter = 10,
    training_func = "train_stereo_detection_VCM", # revised jdc
    test_func = "test_stereo_detection_VCM", # revised jdc
    evaluate_func = "evaluate_kitti_obj_VCM",
    pre_epoches = 20 # revised jdc [load yolo3dstereo training compression]
)
...
## path
path = edict()
path.pretrained_checkpoint = "./workdirs_2/Stereo3D/checkpoint/Stereo3D_best.pth"
# path.pretrained_checkpoint = "./workdirs/Stereo3D/checkpoint/Stereo3D_VCM_64_from256.pth"
...
path.project_path = "./workdirs_2" # or other path for pickle files, checkpoints, tensorboard logging and output files.
...
data = edict(
    batch_size = 4,
    num_workers = 4,
    rgb_shape = (288, 1280, 3), # 这里可以修改需要输入的图像尺寸
...
detector.compression_args = edict(
    lambda_weight=0,
    sigma_weight=7/4
    )

# sigma_weight:1/64; bpp:0.22
# sigma_weight:1/8192; bpp:0.67
...
# 3 卡 跑的实验：新方案的，64点，30-110 workdir_2 2022/11/28 显存：11247