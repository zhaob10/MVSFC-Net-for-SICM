...
def get_label_anno(label_path):
    ...
    #     content = []
    # else:
    content = [line.strip().split(' ') for line in lines if len(line.strip().split(' ')) > 2] # jdc added
...
def get_compression_results(label_folder, image_ids=None):
    if image_ids is None:
        filepaths = pathlib.Path(label_folder).glob('*.txt')
        prog = re.compile(r'^\d{6}.txt$')
        filepaths = filter(lambda f: prog.match(f.name), filepaths)
    ...
    for idx in image_ids:
        image_idx = get_image_index_str(idx)
        label_filename = label_folder / (image_idx + '.txt')
        results = get_compression_result(label_filename)
        bpps.append(results['bpp_total'])
        dists.append(results['dist_total'])
    bpp_mean = np.mean(bpps)
    dist_mean = np.mean(dists)
    text = ''
    text += print_str(f'avg_bpp:  {bpp_mean}')  + print_str(f'avg_dist:  {dist_mean}')
    return text

def get_compression_result(label_path):
    annotations = {}
    annotations.update({
        'bpp_total': [],
        'dist_total': []
    ...
    annotations['bpp_total'] = np.array([float(x[0]) for x in content])
    annotations['dist_total'] = np.array([float(x[1]) for x in content])
    return annotations
