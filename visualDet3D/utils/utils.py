...
class PictursLogger():
    ...
    def reset(self):
        self.pcitures_stats = {} # each will be 
    
    def update(self, pcitures_stats):
        for key in pcitures_stats:
            self.pcitures_stats[key]=pcitures_stats[key]

    def log(self, step):
        for key in self.pcitures_stats:
            name = key + '/' + self.data_split
            self.recorder.add_images(name, self.pcitures_stats[key].clone().cpu(), step)
...
def cfg_from_file(cfg_filename:str)->EasyDict:
    ...
    with tempfile.TemporaryDirectory() as temp_config_dir: # 产生临时目录
        temp_config_file = tempfile.NamedTemporaryFile(dir=temp_config_dir, suffix='.py') # 临时文件
        temp_config_name = os.path.basename(temp_config_file.name) # 临时文件名
        ...
        sys.path.insert(0, temp_config_dir) # 将临时目录放在python路径下可以被检索
