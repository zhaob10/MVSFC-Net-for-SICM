import sys
sys.path.append('/home/zhao/SFCM-Net/Stereo3D_VCM/Stereo3D_VCM_Master')
...
YoloStereo3DCore_VCM, YoloStereo3DCore_VCM_Ablation, YoloStereo3DCore_VCM_Ablation_2, YoloStereo3DCore_VCM_MFCNet
...
class Stereo3D_VCM(nn.Module):
    ...
    def test_forward(self, left_images, right_images, P2, P3, entropy_coding = False): # Actual entropy coding flag
        ...
        if entropy_coding:
            ...
            input = torch.cat([left_images, right_images], dim=1)
            output_dict = self.core(input)
...
if __name__ == "__main__":
    ...
    config = '/home/zhao/SFCM-Net/Stereo3D_VCM/Stereo3D_VCM_Master/config/Stereo3D_VCM.py'
    #config = '/media/D/visualDet3D-master/config/Stereo3D_VCM.py'
    ...
    a = Stereo3D_VCM(cfg.detector, cfg.data.train_augmentation).cuda()
    input = (x, y, p2, p3, 0)
    flops, params = profile(a, inputs = (input, ), verbose=False)
