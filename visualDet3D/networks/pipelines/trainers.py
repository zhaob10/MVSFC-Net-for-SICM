...
@PIPELINE_DICT.register_module
def train_stereo_detection_VCM(data, module:nn.Module,
                     optimizer:optim.Optimizer,
                     aux_optimizer:optim.Optimizer,
                     writer:SummaryWriter=None, 
                     loss_logger:LossLogger=None, 
                     compression_logger:LossLogger=None, 
                     pictures_logger:LossLogger=None, 
                     global_step:int=None,
                     epoch_num:int=None,
                     cfg:EasyDict=EasyDict(),
                     stage:str='pretrain'):
    ...
    # Feed to the network
    classification_loss, regression_loss, rd_loss, aux_loss, loss_dict, bpps, psnrs, pictures = module(
            [left_images.cuda().float().contiguous(), right_images.cuda().float().contiguous(),
            left_images.new(annotation).cuda(),
            P2.cuda(), P3.cuda(),
            disparity.cuda().contiguous()]
    ...
    if not compression_logger is None:
        compression_logger.update(bpps)
        compression_logger.update(psnrs)

    if not pictures_logger is None:
        pictures_logger.update(pictures)

    del bpps, psnrs, pictures

    if not optimizer is None:
        loss = classification_loss + regression_loss + cfg.detector.compression_args.sigma_weight * rd_loss
        # loss = cfg.detector.compression_args.sigma_weight * (classification_loss + regression_loss) +  rd_loss
        # loss =  classification_loss + regression_loss
