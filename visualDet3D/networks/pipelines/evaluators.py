...
from visualDet3D.evaluator.kitti.evaluate import *
...
from visualDet3D.data.kitti.utils import write_result_to_file, write_result_to_file_VCM
...
@PIPELINE_DICT.register_module
@torch.no_grad()
def evaluate_kitti_obj_VCM(cfg:EasyDict, 
                       model:nn.Module,
                       dataset_val:Sized,
                       writer:SummaryWriter,
                       epoch_num:int,
    ...
    # for index in range(len(dataset_val)):
        # 需要将bpp, psnr 写入 result path
        # index = 248
        test_one_VCM(cfg, index, dataset_val, model, test_func, backprojector, projector, result_path) # 在result_path里面根据index写出结果
    if "is_running_test_set" in cfg and cfg["is_running_test_set"]:
        print("Finish evaluation.")
        return
    # 在不同的衡量指标下，总结实验性能
    result_texts = evaluate_VCM(
        label_path=os.path.join(cfg.path.data_path, 'label_2'),
        result_path=result_path,
        label_split_file=cfg.data.val_split_file,
        current_classes=[i for i in range(len(cfg.obj_types))],
    ...
    # 将compression results 读出来
    if writer is not None:
        writer.add_text("validation result compression", result_texts[0].replace(' ', '&nbsp;').replace('\n', '  \n'), epoch_num + 1)
    print(result_texts[0]) # 

    for class_index, result_text in enumerate(result_texts[1:]):
        if writer is not None:
            writer.add_text("validation result {}".format(class_index), result_text.replace(' ', '&nbsp;').replace('\n', '  \n'), epoch_num + 1)
        print(result_text)

def test_one_VCM(cfg, index, dataset, model, test_func, backprojector:BackProjection, projector:BBox3dProjector, result_path):
    data = dataset[index]
    if isinstance(data['calib'], list):
        P2 = data['calib'][0]
    else:
    ...
    # 得到数值
    scores, bbox, obj_names, bpp_total, dist_total = test_func(collated_data, model, None, cfg=cfg)
    ...
    if bbox.shape[1] > 4: # run 3D
        ...
        write_result_to_file_VCM(result_path, index, scores, bbox_2d, bpp_total, dist_total, bbox_3d_state_3d, thetas, obj_names)
    else:
        if "crop_top" in cfg.data.augmentation and cfg.data.augmentation.crop_top is not None:
            crop_top = cfg.data.augmentation.crop_top
        elif "crop_top_height" in cfg.data.augmentation and cfg.data.augmentation.crop_top_height is not None:
