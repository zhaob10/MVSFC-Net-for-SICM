...
def write_result_to_file(base_result_path:str, 
    ...
    """    
    name = "%06d" % index # itn 0 -> str "000000"
    text_to_write = ""
...
def write_result_to_file_VCM(base_result_path:str, 
                        index:int, scores, bbox_2d, bpp_total, dist_total, bbox_3d_state_3d=None, thetas=None, obj_types=['Car', 'Pedestrian', 'Cyclist'], threshold=0.4):
    """Write Kitti prediction results of one frame to a file 
    ...
    text_to_write += ('{:.6f} {:.6f} \n'.format(bpp_total, dist_total)) # added jdc
    file.write(text_to_write)
    file.close()
